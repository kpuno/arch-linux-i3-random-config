...
// @version	7.0
// @namespace	http://www.icedivx.com
// @description	Enables video streaming from Mediafire, 2shared, and all file hosts based on the Xfilesharing script. Streams via icedivx.com.
...
function iceVerify(url){
  ...
  var con = IQS_container(),
	...
	iframe.src = document.location.protocol+'//www.icefilms.info/membersonly/components/com_iceplayer/verify.php?url='+escape(url);
...
function bad_content(content){
	...
	var isBad = content.match(/(does not exist|has been removed|has been deleted|is no longer available|Copyright Infringement|deleted for DMCA|due to inactivity or DMCA|file expired or deleted|File Not Found|File was removed|No such file with this filename|No such user exist|Reason of deletion|The file expired|The file was deleted|file deleted\.)/i);
	if (isBad) {
